...
/// `r + r₂·ℓ` fits in 48 bytes: `r < ℓ < 2²⁵³` and `r₂·ℓ ≤ 2⁶⁴·2²⁵³ = 2³¹⁷`
/// (40 bytes), so their sum is under 320 bits — 48 leaves carry headroom.
const ED_BLINDED_SCALAR_BYTES: usize = 48;

/// Compute `r' = r + r₂·ℓ` little-endian, where `ℓ` is the Ed25519 group order
/// ([`Q_BYTES`]) and `r₂` is a 64-bit blinder. Since `ℓ·G = 𝒪`, `r'·G = r·G`,
...
/// averaging. Schoolbook; the product is `u128` so the 64-bit `r₂` limb times a
/// byte of `ℓ` can't overflow.
fn compute_blinded_ed_scalar(
    ...
    for i in 0..32 {
        let prod = r2 * (Q_BYTES[i] as u128) + carry;
        out[i] = prod as u8;
        carry = prod >> 8;
    }
...
///   `Z`. Randomizes the output per call; a weak or constant `Z` degrades to the
///   deterministic RFC 8032 nonce (`prefix ‖ M` is still hashed) — never to
///   nonce reuse, so a broken RNG can't leak the key.
    ...
    <T as const_num_traits::ToBytes>::Bytes: zeroize::Zeroize,
{
    ...
    let r_blinded = compute_blinded_ed_scalar(&r_le, u64::from_le_bytes(r2_bytes));
