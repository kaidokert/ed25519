use modmath::{basic_mod_add, basic_mod_exp, basic_mod_inv, basic_mod_mul, basic_mod_sub};

#[cfg(feature = "montgomery")]
use modmath::{NPrimeMethod, basic_montgomery_mod_exp_with_method};

#[cfg(feature = "std")]
use log::info;

#[cfg(feature = "defmt")]
use defmt::info;

#[cfg(all(feature = "montgomery", feature = "montgomery-euclidean"))]
const METHOD: NPrimeMethod = NPrimeMethod::ExtendedEuclidean;

#[cfg(all(feature = "montgomery", feature = "montgomery-hensels"))]
const METHOD: NPrimeMethod = NPrimeMethod::HenselsLifting;

#[cfg(all(
    feature = "montgomery",
    not(feature = "montgomery-euclidean"),
    not(feature = "montgomery-hensels")
))]
const METHOD: NPrimeMethod = NPrimeMethod::ExtendedEuclidean; // Default to ExtendedEuclidean if no method specified

use crate::{BrigIntStrict, Point};
use crate::{D_BYTES, G_T_BYTES, G_X_BYTES, G_Y_BYTES, MODP_SQRT_M1_BYTES, P_BYTES, Q_BYTES};

// Basic trait that just requires Copy + basic arithmetic
pub trait CoreIntBasic:
    Copy
    + Clone
    + core::cmp::PartialOrd
    + core::cmp::PartialEq
    + num_traits::One
    + num_traits::Zero
    + core::ops::Add<Output = Self>
    + core::ops::Sub<Output = Self>
    + core::ops::Mul<Output = Self>
    + core::ops::Div<Output = Self>
    + core::ops::Rem<Output = Self>
    + core::ops::RemAssign<Self>
    + core::ops::BitAnd<Output = Self>
    + core::ops::Shr<usize, Output = Self>
    + core::ops::Shl<usize, Output = Self>
    + core::ops::ShrAssign<usize>
    + num_traits::ops::wrapping::WrappingAdd
    + num_traits::ops::wrapping::WrappingSub
{
}

impl<T> CoreIntBasic for T where
    T: Copy
        + Clone
        + core::cmp::PartialOrd
        + core::cmp::PartialEq
        + num_traits::One
        + num_traits::Zero
        + core::ops::Add<Output = T>
        + core::ops::Sub<Output = T>
        + core::ops::Mul<Output = T>
        + core::ops::Div<Output = T>
        + core::ops::Rem<Output = T>
        + core::ops::RemAssign<T>
        + core::ops::BitAnd<Output = T>
        + core::ops::Shr<usize, Output = T>
        + core::ops::Shl<usize, Output = T>
        + core::ops::ShrAssign<usize>
        + num_traits::ops::wrapping::WrappingAdd
        + num_traits::ops::wrapping::WrappingSub
{
}

pub trait BrigIntBasic: CoreIntBasic + BrigIntStrict {}

impl<T> BrigIntBasic for T where T: CoreIntBasic + BrigIntStrict {}

fn print_value<T: BrigIntBasic>(label: &str, value: T) {
    let mut buffer = [0u8; 128];
    let bytes = value.to_bytes_le(&mut buffer);

    // Trim trailing zeros to show only significant bytes
    let trimmed_bytes = if let Some(last_nonzero) = bytes.iter().rposition(|&b| b != 0) {
        &bytes[..=last_nonzero]
    } else {
        &bytes[..1] // Keep at least one byte even if value is zero
    };

    let hex_string: String = trimmed_bytes.iter().map(|b| format!("{:02x}", b)).collect();
    info!("{}: {}", label, hex_string);
}

fn recover_x<T: BrigIntBasic>(y: T, sign: u8, p: T, d: T) -> Option<T> {
    let one = T::one();

    // First compute y² mod p
    let y2 = basic_mod_mul(y, y, p);

    // left = (y² - 1) mod p
    let left = basic_mod_sub(y2, one, p);

    // denom_raw = (d * y²) mod p
    let denom_raw = basic_mod_mul(d, y2, p);

    // denom = (denom_raw + 1) mod p
    let denom = basic_mod_add(denom_raw, one, p);

    info!("recovering x");
    print_value("denom", denom);
    print_value("p", p);

    // Use direct modular inverse instead of Fermat's little theorem
    let inv_denom = basic_mod_inv(denom, p).unwrap();

    // Finally, x2 = left * inv_denom mod p
    let x2 = basic_mod_mul(left, inv_denom, p);

    if x2 == T::zero() {
        if sign > 0 { None } else { Some(T::zero()) }
    } else {
        let two = one + one;
        let three = two + one;
        let four = two + two;
        let eight = four + four;

        let p3 = p + three;
        let exp = p3 / eight;

        let mut x = {
            #[cfg(feature = "montgomery")]
            {
                basic_montgomery_mod_exp_with_method(x2, exp, p, METHOD).unwrap()
            }
            #[cfg(not(feature = "montgomery"))]
            {
                basic_mod_exp(x2, exp, p)
            }
        };

        // Use precomputed modp_sqrt_m1 = 2^((p-1)/4) mod p instead of computing it
        let modp_sqrt_m1 = T::from_bytes_le(&MODP_SQRT_M1_BYTES);

        let tmp1 = basic_mod_mul(x, x, p);
        let tmp2 = basic_mod_sub(tmp1, x2, p);
        if tmp2 != T::zero() {
            x = basic_mod_mul(x, modp_sqrt_m1, p);
        }

        let tmp1 = basic_mod_mul(x, x, p);
        let tmp2 = basic_mod_sub(tmp1, x2, p);
        if tmp2 != T::zero() {
            return None;
        }

        let weird = (x & one == one) as u8 != sign;
        if weird {
            x = p - x;
        }

        Some(x)
    }
}

fn decompress_edward_point<T: BrigIntBasic>(k: [u8; 32], p: T, d: T) -> Option<Point<T>> {
    let mut k_list = k;
    let sign = k_list[k.len() - 1] >> 7;
    k_list[k.len() - 1] &= 0b01111111;
    let y = T::from_bytes_le(&k_list[0..32]);

    if y > p {
        None
    } else {
        let x = recover_x(y, sign, p, d);
        x.map(|x_val| (x_val, y, T::one(), basic_mod_mul(x_val, y, p)))
    }
}

fn concat_emul<'a>(slices: &[&[u8]], storage: &'a mut [u8]) -> &'a [u8] {
    let total_len = slices.iter().map(|s| s.len()).sum();
    let mut offset = 0;
    for slice in slices {
        storage[offset..offset + slice.len()].copy_from_slice(slice);
        offset += slice.len();
    }
    &storage[..total_len]
}

fn sha512_modq<T: BrigIntBasic>(msg: &[u8], q: T) -> T {
    let mut compact_sha = hmac_sha512::Hash::new();
    compact_sha.update(msg);
    let finalized = compact_sha.finalize();
    let hash = finalized.as_slice();

    let result_nomodq = T::from_bytes_le(&hash[0..64]);
    result_nomodq % q
}

fn point_add<T: BrigIntBasic>(pp: &Point<T>, qq: &Point<T>, p: T, d: T) -> Point<T> {
    // a = (pp.1 - pp.0) * (qq.1 - qq.0) mod p
    let pp1_sub_pp0 = basic_mod_sub(pp.1, pp.0, p);
    let qq1_sub_qq0 = basic_mod_sub(qq.1, qq.0, p);
    let a = basic_mod_mul(pp1_sub_pp0, qq1_sub_qq0, p);

    // b = (pp.1 + pp.0) * (qq.1 + qq.0) mod p
    let pp1_add_pp0 = basic_mod_add(pp.1, pp.0, p);
    let qq1_add_qq0 = basic_mod_add(qq.1, qq.0, p);
    let b = basic_mod_mul(pp1_add_pp0, qq1_add_qq0, p);

    // c = 2 * pp.3 * qq.3 * d mod p
    let two = T::one() + T::one();
    let pp3_mul_qq3 = basic_mod_mul(pp.3, qq.3, p);
    let c_temp = basic_mod_mul(two, pp3_mul_qq3, p);
    let c = basic_mod_mul(c_temp, d, p);

    // d_val = 2 * pp.2 * qq.2 mod p
    let pp2_mul_qq2 = basic_mod_mul(pp.2, qq.2, p);
    let d_val = basic_mod_mul(two, pp2_mul_qq2, p);

    // x3 = (b - a) * (d_val - c) mod p
    let b_sub_a = basic_mod_sub(b, a, p);
    let d_sub_c = basic_mod_sub(d_val, c, p);
    let x3 = basic_mod_mul(b_sub_a, d_sub_c, p);

    // y3 = (d_val + c) * (b + a) mod p
    let d_add_c = basic_mod_add(d_val, c, p);
    let b_add_a = basic_mod_add(b, a, p);
    let y3 = basic_mod_mul(d_add_c, b_add_a, p);

    // z3 = (d_val - c) * (d_val + c) mod p
    let z3 = basic_mod_mul(d_sub_c, d_add_c, p);

    // t3 = (b - a) * (b + a) mod p
    let t3 = basic_mod_mul(b_sub_a, b_add_a, p);

    (x3, y3, z3, t3)
}

fn point_mul<T: BrigIntBasic>(s: T, pp: &Point<T>, p: T, d: T) -> Point<T> {
    let mut q = (T::zero(), T::one(), T::one(), T::zero());
    let mut current = *pp;
    let mut remaining = s;

    while remaining > T::zero() {
        if remaining & T::one() == T::one() {
            q = point_add(&q, &current, p, d);
        }
        current = point_add(&current, &current, p, d);
        remaining >>= 1;
    }
    q
}

fn point_equal<T: BrigIntBasic>(pp: &Point<T>, qq: &Point<T>, p: T) -> bool {
    // term1 = pp.0 * qq.2 mod p
    let term1 = basic_mod_mul(pp.0, qq.2, p);

    // term2 = qq.0 * pp.2 mod p
    let term2 = basic_mod_mul(qq.0, pp.2, p);

    // term3 = pp.1 * qq.2 mod p
    let term3 = basic_mod_mul(pp.1, qq.2, p);

    // term4 = qq.1 * pp.2 mod p
    let term4 = basic_mod_mul(qq.1, pp.2, p);

    // Check if term1 - term2 == 0 (mod p) and term3 - term4 == 0 (mod p)
    let diff1 = basic_mod_sub(term1, term2, p);
    let diff2 = basic_mod_sub(term3, term4, p);

    diff1 == T::zero() && diff2 == T::zero()
}

pub fn verify<T: BrigIntBasic>(public: [u8; 32], msg: &[u8], signature: [u8; 64]) -> bool {
    let total_start = std::time::Instant::now();

    let p = T::from_bytes_le(&P_BYTES);
    let d = T::from_bytes_le(&D_BYTES);
    let q = T::from_bytes_le(&Q_BYTES);

    let g = (
        T::from_bytes_le(&G_X_BYTES),
        T::from_bytes_le(&G_Y_BYTES),
        T::one(),
        T::from_bytes_le(&G_T_BYTES),
    );

    let start = std::time::Instant::now();
    let aa = decompress_edward_point(public, p, d);
    info!(
        "TIMING: decompress_edward_point(public) took {:.3}ms",
        start.elapsed().as_secs_f64() * 1000.0
    );
    if aa.is_none() {
        return false;
    }
    let aa = aa.unwrap();
    let rrs: [u8; 32] = signature[0..32]
        .try_into()
        .expect("Invalid signature length");

    let start = std::time::Instant::now();
    let rr = decompress_edward_point(rrs, p, d);
    info!(
        "TIMING: decompress_edward_point(rrs) took {:.3}ms",
        start.elapsed().as_secs_f64() * 1000.0
    );
    if rr.is_none() {
        return false;
    }
    let rr = rr.unwrap();

    let s_bytes: [u8; 32] = signature[32..64]
        .try_into()
        .expect("invalid signature length");

    let s = T::from_bytes_le(&s_bytes);
    if s >= q {
        return false;
    }
    let rrs = rrs.as_slice();
    let public = public.as_slice();
    let mut storage = [0u8; 128];

    let concat = concat_emul(&[rrs, public, msg], &mut storage);
    let start = std::time::Instant::now();
    let h = sha512_modq(&concat, q);
    info!(
        "TIMING: sha512_modq took {:.3}ms",
        start.elapsed().as_secs_f64() * 1000.0
    );

    let start = std::time::Instant::now();
    let sbb = point_mul(s, &g, p, d);
    info!(
        "TIMING: point_mul(s, g) took {:.3}ms",
        start.elapsed().as_secs_f64() * 1000.0
    );

    let start = std::time::Instant::now();
    let haa = point_mul(h, &aa, p, d);
    info!(
        "TIMING: point_mul(h, aa) took {:.3}ms",
        start.elapsed().as_secs_f64() * 1000.0
    );

    let start = std::time::Instant::now();
    let second_point = point_add(&rr, &haa, p, d);
    info!(
        "TIMING: point_add(rr, haa) took {:.3}ms",
        start.elapsed().as_secs_f64() * 1000.0
    );

    let start = std::time::Instant::now();
    let result = point_equal(&sbb, &second_point, p);
    info!(
        "TIMING: point_equal took {:.3}ms",
        start.elapsed().as_secs_f64() * 1000.0
    );

    info!(
        "TIMING: TOTAL verify() took {:.3}ms",
        total_start.elapsed().as_secs_f64() * 1000.0
    );
    result
}
