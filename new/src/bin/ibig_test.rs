// Minimal test for ibig-patched backend
// Run with: cargo run --bin ibig_test --features="ibig-patched" --release
...
fn main() {
    println!("=== ibig-patched Ed25519 Test ===");

    // Test strict mode (should work according to modmath compatibility)
    println!("Testing STRICT mode...");
    let strict_result =
        ed25519n::strict::verify::<ibig_patched::UBig>(PUBLIC_KEY, MESSAGE, SIGNATURE);
    println!(
        "STRICT result: {}",
        if strict_result {
            "✅ PASS"
    ...
    // Note: This should fail to compile because ibig_patched::UBig doesn't implement Copy
    // let basic_result = ed25519n::basic::verify::<ibig_patched::UBig>(PUBLIC_KEY, MESSAGE, SIGNATURE);
    println!("BASIC result: ❌ SKIPPED (no Copy trait for heap-allocated types)");

    if strict_result && constrained_result {
        println!("\n🎉 ibig-patched working in 2/2 expected modes!");
    } else {
        println!("\n❌ ibig-patched has issues");
    }
}
